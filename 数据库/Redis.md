...
#### AOF持久化
AOF：append of file，将数据以追加的方式写入aof日志文件，因为是追加写，性能高，写入的是写相关命令。当redis重启后，先从aof读取命令，重新写入内存。当AOF文件过大时，会触发AOF的后台重写，只保留最后的写命令，达到压缩文件的目的。
- 优点：数据完整性高
- 缺点：aof文件可能过大，恢复时间长

![AOF写流程](./imgs/Redis-AOF.webp)

#### RDB快照持久化
RDB快照：将某一时刻的内存中的数据写入磁盘，恢复时直接将该数据恢复到内存中
- 优点：文件小，二进制数据文件
- 缺点：受限与RDB的频率，频率高了影响系统性能，频率低了丢数据

#### 混合持久化
混合持久化就是结合aof和RDB各自的优点来尽可能提高持久化性能和数据的准确性。
在AOF日志触发重写时，fork的子进程会先将主线程内存中的数据以RDB的方式写入新AOF文件中，主线程此时的写命令会被记录到重写缓冲区，重写缓冲区中的增量写命令会被追加到这个新的AOF文件的末尾，和RDB一起组成一个开头是RDB全量数据，结尾是AOF增量命令的混合日志。

### 主从架构
一主多从，单独其线程做主节点和从节点的数据同步。但是恢复系统时需要手动指定主节点

### 哨兵系统
哨兵监听每个节点，当主节点挂掉后，通知从节点，并根据一致性算法选出新的主节点

### 分片集群
多个主从节点构成分片集群，集群的每个几点都是一个主从架构的小集群，通过负载均衡提高系统的高性能，高可用，高并发。

## Redis过期删除策略
- 定时删除：在设置过期时间时，同时设置过期事件，当过期时触发过期事件删除
- 惰性删除：当访问的时候检测是否过期，过期则删除
- 定期删除：每隔一端时间，随机选取一些key，并判断是否过期，过期则删除

![Rdisa惰性删除](./imgs/Redis惰性删除.webp)
![Rdisa定期删除](./imgs/Redis定期删除流程.webp)

## Redis内存淘汰策略
- noeviction：当运行内存超过了设置的最多值时，不进行任何数据淘汰，新写入触发OOM
- 在设置了过期时间的键中淘汰：
  - volatile-random：随机淘汰设置了过期时间的任意键
  - volatile-ttl：优先淘汰更早过期的键
  - volatile-lru：淘汰所有设置了过期时间的键中，最久未使用的键（前期使用次数多，后期未使用）。选取一些键，比较最后实用时间，
  - volatile-lfu：淘汰所有设置了过期时间的键中，最少使用的键（使用次数最少）
- 在所有键中淘汰：
  - allkeys-random：随机淘汰任意键
  - allkeys-lru：所有键中，最久未使用的键
  - allkeys-lfu：所有键中，最少使用的键

![Redis内存淘汰策略](./imgs/内存淘汰策略.webp)